...
public class ClothDropdownSelectEntry<V> extends TooltipListEntry<V> {
	...
	public ClothDropdownSelectEntry(Text fieldName, V value, Text resetButtonText, Supplier<Optional<String[]>> tooltipSupplier, boolean requiresRestart, Supplier<V> defaultValue, Consumer<V> saveConsumer, List<V> options, Function<V, Text> valueConverter) {
		super(fieldName.asFormattedString(), tooltipSupplier, requiresRestart);
		...
		mainButton = new ButtonWidget(0, 0, 150, 20, resetButtonText.asFormattedString(), button -> {
			optionsVisible = !optionsVisible;
			scrollOffset = 0;
		});
		resetButton = new ButtonWidget(0, 0, MinecraftClient.getInstance().textRenderer.getStringWidth(resetButtonText.asString()) + 6, 20, resetButtonText.asFormattedString(), button -> {
	...
	public void setValue(V value) {
		this.value = value;
		if (value == null) {
			mainButton.setMessage(I18n.translate("tweed4.cloth.dropdown.empty"));
		} else {
			mainButton.setMessage(valueConverter.apply(value).asFormattedString());
		}
	}
	...
	public void render(int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean isHovered, float delta) {
		...
		if (MinecraftClient.getInstance().textRenderer.isRightToLeft()) {
			MinecraftClient.getInstance().textRenderer.drawWithShadow(getFieldName(), window.getScaledWidth() - x - MinecraftClient.getInstance().textRenderer.getStringWidth(getFieldName()), y + 6, getPreferredTextColor());
			resetButton.x = x;
			mainButton.x = x + resetButton.getWidth() + 1;
		} else {
			MinecraftClient.getInstance().textRenderer.drawWithShadow(getFieldName(), x, y + 6, getPreferredTextColor());
		...
		if (optionsVisible) {
			...
			for (int i = scrollOffset; i < scrollOffset + length; i++) {
				MinecraftClient.getInstance().textRenderer.draw(valueConverter.apply(options[i]).asFormattedString(), mainButton.x + 3F, top + (i - scrollOffset) * 14 + 3,
