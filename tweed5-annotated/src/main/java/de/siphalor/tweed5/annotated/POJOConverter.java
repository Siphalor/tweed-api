...
import de.siphalor.tweed5.config.constraints.AnnotationConstraint;
import de.siphalor.tweed5.config.entry.AbstractValueConfigEntry;
import de.siphalor.tweed5.config.entry.ConfigEntry;
import de.siphalor.tweed5.config.entry.ValueConfigEntry;
import de.siphalor.tweed5.config.fixers.ConfigEntryFixer;
import de.siphalor.tweed5.registry.TweedIdentifier;
import de.siphalor.tweed5.config.value.ReferenceConfigValue;
...
import de.siphalor.tweed5.config.value.serializer.NullableSerializer;
...
import de.siphalor.tweed5.util.ReflectionUtil;
...
public class POJOConverter {
	...
	public static ConfigFile toConfigFile(Object pojo, String fallbackFileName) throws RuntimeException {
		...
		rootCategory.setScope(ReloadScope.ENUM.valueOf(tweedConfig.scope()));
		rootCategory.setEnvironment(ReloadEnvironment.ENUM.valueOf(tweedConfig.environment()));
	...
	private static String processConfigEntry(ConfigEntry<?> entry, Field field, CaseFormat casing) {
		...
		if (field.isAnnotationPresent(AConfigEntry.class)) {
			...
			entry.setScope(ReloadScope.ENUM.valueOf(configData.scope()));
			entry.setEnvironment(ReloadEnvironment.ENUM.valueOf(configData.environment()));
